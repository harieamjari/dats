...
// this API works by first initializing an sf_reverb_state_st structure, then
// using it to process a sample in chunks
//
...
//
// notice that sf_reverb_process will change a lot of the member variables
...
// completely arbitrary from the reverb's perspective
//
// ---
//
// non-convolution based reverb effects are made up from a lot of smaller
// effects
//
// each reverb algorithm's sound is based on how the designers setup these
// smaller effects and chained them together
//
// this particular setup is based on Progenitor2 from Freeverb3, and uses the
...
//
// each of these components is broken into their own structures (sf_rv_*), and
// the reverb effect uses these in the final state structure
// (sf_reverb_state_st)
//
// each component is designed to work one step at a time, so any size sample can
// be streamed through in one pass
...
// biquad
// note: we don't use biquad.c because we want to step through the sound one
// sample at a time, one
//       channel at a time
typedef struct {
...
// fractal noise cache
// noise buffer size; must be a power of 2 because it's generated via fractal
// generator
...
typedef struct {
  ...
  int count; // number of samples generated so far (used to apply small
...
typedef struct {
  ...
  float buf1[SF_REVERB_AP3S1 + SF_REVERB_AP3M1], buf2[SF_REVERB_AP3S2],
      buf3[SF_REVERB_AP3S3];
} sf_rv_allpass3_st;
...
//
// note: this is about 2megs, so you might not want to throw these around
// willy-nilly
typedef struct {
...
void sf_presetreverb(sf_reverb_state_st *state, int rate,
