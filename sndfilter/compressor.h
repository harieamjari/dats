...
//
// this API works by first initializing an sf_compressor_state_st structure,
// then using it to process a sample in chunks
//
...
//
// notice that sf_compressor_process will change a lot of the member variables
// inside of the state structure, since these values must be carried over across
// chunk boundaries
//
// also notice that the choice to divide the sound into chunks of 128 samples is
// completely arbitrary from the compressor's perspective, however, the size
// should be divisible by the SPU value below (defaults to 32):

// maximum number of samples in the delay buffer
#define SF_COMPRESSOR_MAXDELAY 1024

// samples per update; the compressor works by dividing the input chunks into
// even smaller sizes, and performs heavier calculations after each mini-chunk
// to adjust the final envelope
...
typedef struct {
  // user can read the metergain state variable after processing a chunk to see
  // how much dB the compressor would have liked to compress the sample; the
  // meter values aren't used to shape the sound in any way, only used for
  // output if desired
  float metergain;

  // everything else shouldn't really be mucked with unless you read the
  // algorithm and feel comfortable
  float meterrelease;
  float threshold;
  float knee;
  float linearpregain;
...
void sf_simplecomp(
    ...
    float pregain, // dB, amount to boost the signal before applying compression
                   // [0 to 100]
    float threshold, // dB, level where compression kicks in [-100 to 0]
    float knee,      // dB, width of the knee [0 to 40]
    float ratio, // unitless, amount to inversely scale the output when applying
                 // comp [1 to 20]
...
void sf_advancecomp(
    ...
    int rate, float pregain, float threshold, float knee, float ratio,
    float attack, float release,
    // these are the advanced parameters:
    float predelay,     // seconds, length of the predelay buffer [0 to 1]
    float releasezone1, // release zones should be increasing between 0 and 1,
                        // and are a fraction
    float releasezone2, //  of the release time depending on the input dB --
                        //  these parameters define
    float releasezone3, //  the adaptive release curve, which is discussed in
    ...
    float
        wet // amount to apply the effect [0 completely dry to 1 completely wet]
);
