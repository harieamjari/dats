...
// components are in the basic format of `<component>_make` to initialize a
// structure and
...
// biquad
//
static inline void biquad_makeLPF(sf_rv_biquad_st *biquad, int rate, float freq,
                                  float bw) {
...
static inline void biquad_makeLPFQ(sf_rv_biquad_st *biquad, int rate,
                                   float freq, float bw) {
  freq = clampf(freq, 0, rate / 2);
  float omega = 2.0f * (float)M_PI * freq / (float)rate;
  float cs = cosf(omega);
  float alpha =
      sinf(omega) * 2.0f * bw; // different alpha calculation than makeLPF above
  float a0inv = 1.0f / (1.0f + alpha);
  biquad->b0 = a0inv * (1.0f - cs) * 0.5f;
  biquad->b1 = 2.0f * biquad->b0;
  biquad->b2 = biquad->b0;
...
static inline void biquad_makeAPF(sf_rv_biquad_st *biquad, int rate, float freq,
                                  float bw) {
  freq = clampf(freq, 0, rate / 2);
  float omega = 2.0f * (float)M_PI * freq / (float)rate;
  float sn = sinf(omega);
...
static inline float biquad_step(sf_rv_biquad_st *biquad, float v) {
  float out = v * biquad->b0 + biquad->xn1 * biquad->b1 +
              biquad->xn2 * biquad->b2 - biquad->yn1 * biquad->a1 -
              biquad->yn2 * biquad->a2;
...
// earlyref
//
static inline void earlyref_make(sf_rv_earlyref_st *earlyref, int rate,
                                 float factor, float width) {
  static const sf_sample_st delaytbl[18] = {
      // seconds to look backwards
      {0.0043f, 0.0053f}, {0.0215f, 0.0225f}, {0.0225f, 0.0235f},
      {0.0268f, 0.0278f}, {0.0270f, 0.0290f}, {0.0298f, 0.0288f},
      {0.0458f, 0.0468f}, {0.0485f, 0.0475f}, {0.0572f, 0.0582f},
      {0.0587f, 0.0577f}, {0.0595f, 0.0575f}, {0.0612f, 0.0622f},
      {0.0707f, 0.0697f}, {0.0708f, 0.0718f}, {0.0726f, 0.0736f},
      {0.0741f, 0.0751f}, {0.0753f, 0.0763f}, {0.0797f, 0.0817f}};
...
static inline sf_sample_st earlyref_step(sf_rv_earlyref_st *earlyref,
                                         sf_sample_st input) {
  ...
  for (int i = 0; i < 18; i++) {
    wetL +=
        gaintbl[i].L * delay_get(&earlyref->delayPWL, earlyref->delaytblL[i]);
    wetR +=
        gaintbl[i].R * delay_get(&earlyref->delayPWR, earlyref->delaytblR[i]);
  }

  float L = delay_step(&earlyref->delayRL, input.R + wetR);
  L = biquad_step(&earlyref->allpassXL, L);
  L = biquad_step(&earlyref->allpassL,
                  earlyref->wet1 * wetL + earlyref->wet2 * L);
  ...
  R = biquad_step(&earlyref->allpassR,
                  earlyref->wet1 * wetR + earlyref->wet2 * R);
...
// oversample
//
static inline void oversample_make(sf_rv_oversample_st *oversample,
                                   int factor) {
...
static inline void oversample_stepup(sf_rv_oversample_st *oversample,
...
static inline float oversample_stepdown(sf_rv_oversample_st *oversample,
                                        float *input) {
  if (oversample->factor == 1)
    return input[0];
  ...
  if (noise->pos >= SF_REVERB_NS) {
    ...
    while (len > 1) {
      ...
      for (int i = tot - 1; i >= 0; i--) {
        ...
        float newv = midpoint +
                     r * (2.0f * randfloat() - 1.0f); // displace by random amt
...
static inline float lfo_step(sf_rv_lfo_st *lfo) {
  ...
  if (lfo->count++ > 100000) {
    // if we've gathered a lot of samples, then it's probably a good idea to
    // make sure our LFO hasn't accumulated a bunch of errors
    lfo->count = 0;
...
// allpass
//
static inline void allpass_make(sf_rv_allpass_st *allpass, int size,
                                float feedback, float decay) {
  allpass->pos = 0;
...
static inline float allpass_step(sf_rv_allpass_st *allpass, float v) {
  ...
  float out =
      allpass->decay * allpass->buf[allpass->pos] - allpass->feedback * v;
...
// allpass2
//
static inline void allpass2_make(sf_rv_allpass2_st *allpass2, int size1,
                                 int size2, float feedback1, float feedback2,
                                 float decay1, float decay2) {
  allpass2->pos1 = 0;
  allpass2->pos2 = 0;
...
static inline float allpass2_step(sf_rv_allpass2_st *allpass2, float v) {
  ...
  float out = allpass2->decay2 * allpass2->buf2[allpass2->pos2] -
              v * allpass2->feedback2;
  v += allpass2->feedback1 * allpass2->buf1[allpass2->pos1];
  allpass2->buf2[allpass2->pos2] =
      allpass2->decay1 * allpass2->buf1[allpass2->pos1] -
...
// allpass3
//
static inline void allpass3_make(sf_rv_allpass3_st *allpass3, int size1,
                                 int msize1, int size2, int size3,
                                 float feedback1, float feedback2,
                                 float feedback3, float decay1, float decay2,
                                 float decay3) {
...
static inline float allpass3_step(sf_rv_allpass3_st *allpass3, float v,
                                  float mod) {
  ...
  float out = allpass3->decay3 * allpass3->buf3[allpass3->pos3] -
              allpass3->feedback3 * v;
  v += allpass3->feedback2 * allpass3->buf2[allpass3->pos2];
  allpass3->buf3[allpass3->pos3] =
      allpass3->decay2 * allpass3->buf2[allpass3->pos2] -
      allpass3->feedback2 * v;
  float tmp =
      allpass3->buf1[rpos2] * mfrac + allpass3->buf1[rpos1] * (1.0f - mfrac);
  v += allpass3->feedback1 * tmp;
  allpass3->buf2[allpass3->pos2] =
      allpass3->decay1 * tmp - allpass3->feedback1 * v;
...
// allpassm
//
static inline void allpassm_make(sf_rv_allpassm_st *allpassm, int size,
                                 int msize, float feedback, float decay) {
...
static inline float allpassm_step(sf_rv_allpassm_st *allpassm, float v,
                                  float mod, float fbmod) {
  ...
  if (rpos2 < 0)
    rpos2 += allpassm->size;
  allpassm->z1 =
      allpassm->buf[rpos2] + mfrac * (allpassm->buf[rpos1] - allpassm->z1);
  allpassm->rpos = (allpassm->rpos + 1) % allpassm->size;
  allpassm->buf[allpassm->wpos] = v + allpassm->z1 * mfeedback;
  v = allpassm->decay * allpassm->z1 -
      allpassm->buf[allpassm->wpos] * mfeedback;
...
// now that all the components are done (thank god), we can start on the actual
// reverb effect

void sf_presetreverb(sf_reverb_state_st *rv, int rate,
                     sf_reverb_preset preset) {
  ...
  struct {
    ...
    float p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, p13, p14, p15, p16;
      ...
      // OSF ERtoLt ERWet Dry ERFac ERWdth Wdth Wet Wander BassB Spin InpLP
      // BasLP DmpLP OutLP RT60  Delay
      {1, 0.40f, -9.0f, -10, 1.6f, 0.7f, 1.0f, -0, 0.27f, 0.15f, 0.7f, 17000,
       500, 7000, 10000, 3.2f, 0.020f},
      {2, 0.30f, -9.0f, -8, 1.0f, 0.7f, 1.0f, -8, 0.32f, 0.25f, 0.7f, 18000,
       600, 9000, 17000, 2.1f, 0.010f},
      {1, 0.30f, -9.0f, -8, 1.0f, 0.7f, 1.0f, -8, 0.27f, 0.20f, 0.5f, 18000,
       600, 7000, 9000, 2.3f, 0.010f},
      {2, 0.30f, -9.0f, -8, 1.2f, 0.7f, 1.0f, -8, 0.27f, 0.20f, 0.7f, 18000,
       500, 8000, 16000, 2.8f, 0.010f},
      {1, 0.30f, -9.0f, -8, 1.2f, 0.7f, 1.0f, -8, 0.25f, 0.15f, 0.5f, 18000,
       500, 6000, 8000, 2.9f, 0.010f},
      {2, 0.20f, -9.0f, -8, 1.4f, 0.7f, 1.0f, -8, 0.17f, 0.20f, 1.0f, 18000,
       400, 9000, 14000, 3.8f, 0.018f},
      {2, 0.20f, -9.0f, -8, 1.5f, 0.7f, 1.0f, -8, 0.20f, 0.20f, 0.5f, 18000,
       400, 5000, 7000, 4.2f, 0.018f},
      {2, 0.70f, -8.0f, -8, 0.7f, -0.4f, 0.8f, -8, 0.20f, 0.30f, 1.6f, 18000,
       1000, 18000, 18000, 0.5f, 0.005f},
      {3, 0.70f, -8.0f, -8, 0.8f, 0.6f, 0.9f, -8, 0.30f, 0.30f, 0.4f, 18000,
       300, 10000, 18000, 0.5f, 0.005f},
      {2, 0.50f, -8.0f, -8, 1.2f, -0.4f, 0.8f, -8, 0.20f, 0.10f, 1.6f, 18000,
       1000, 18000, 18000, 0.8f, 0.008f},
      {2, 0.50f, -8.0f, -8, 1.2f, 0.6f, 0.9f, -8, 0.30f, 0.10f, 0.4f, 18000,
       300, 10000, 18000, 1.2f, 0.016f},
      {2, 0.20f, -8.0f, -8, 2.2f, -0.4f, 0.9f, -8, 0.20f, 0.10f, 1.6f, 18000,
       1000, 16000, 18000, 1.8f, 0.010f},
      {2, 0.20f, -8.0f, -8, 2.2f, 0.6f, 0.9f, -8, 0.30f, 0.10f, 0.4f, 18000,
       500, 9000, 18000, 1.9f, 0.020f},
      {2, 0.50f, -7.0f, -7, 1.2f, -0.4f, 0.8f, -70, 0.20f, 0.10f, 1.6f, 18000,
       1000, 18000, 18000, 0.8f, 0.008f},
      {2, 0.50f, -7.0f, -7, 1.2f, 0.6f, 0.9f, -70, 0.30f, 0.10f, 0.4f, 18000,
       300, 10000, 18000, 1.2f, 0.016f},
      {2, 0.00f, -70.0f, -20, 1.0f, 1.0f, 1.0f, -8, 0.20f, 0.10f, 1.6f, 18000,
       1000, 16000, 18000, 1.8f, 0.000f},
      {2, 0.00f, -70.0f, -20, 1.0f, 1.0f, 1.0f, -8, 0.30f, 0.20f, 0.4f, 18000,
       500, 9000, 18000, 1.9f, 0.000f},
      {2, 0.10f, -16.0f, -15, 1.0f, 0.1f, 1.0f, -5, 0.35f, 0.05f, 1.0f, 18000,
       100, 10000, 18000, 12.0f, 0.000f},
      {2, 0.10f, -16.0f, -15, 1.0f, 0.1f, 1.0f, -5, 0.40f, 0.05f, 1.0f, 18000,
       100, 9000, 18000, 30.0f, 0.000f}

  };

#define CASE(prs, i)                                                           \
  case prs:                                                                    \
    sf_advancereverb(rv, rate, ps[i].osf, ps[i].p1, ps[i].p2, ps[i].p3,        \
                     ps[i].p4, ps[i].p5, ps[i].p6, ps[i].p7, ps[i].p8,         \
                     ps[i].p9, ps[i].p10, ps[i].p11, ps[i].p12, ps[i].p13,     \
                     ps[i].p14, ps[i].p15, ps[i].p16);                         \
    return;
  switch (preset) {
...
void sf_advancereverb(sf_reverb_state_st *rv, int rate, int oversamplefactor,
                      float ertolate, float erefwet, float dry,
                      float ereffactor, float erefwidth, float width, float wet,
                      float wander, float bassb, float spin, float inputlpf,
                      float basslpf, float damplpf, float outputlpf, float rt60,
                      float delay) {
  ...
  lfo_make(&rv->lfo2, osrate,
           sqrtf(100.0f - (10.0f - spin) * (10.0f - spin)) * 0.5f);
  iir1_makeLPF(&rv->lfo2_lpf, osrate, 12.0f);

  static const int diffLc[10] = {617, 535, 434, 347, 218,
                                 162, 144, 122, 109, 74};
  static const int diffRc[10] = {603, 547, 416, 364, 236,
                                 162, 140, 131, 111, 79};
  ...
  for (int i = 0; i < 10; i++) {
    allpassm_make(&rv->diffL[i], nextprime(diffLc[i] * totfactor), msize,
                  -0.78f, 1);
    allpassm_make(&rv->diffR[i], nextprime(diffRc[i] * totfactor), msize,
                  -0.78f, 1);
  }
  ...
  allpassm_make(&rv->dampap1L, nextprime(239 * totfactor), msize, 0.375f,
                decay2);
  allpassm_make(&rv->dampap1R, nextprime(205 * totfactor), msize, 0.375f,
                decay2);
  allpassm_make(&rv->dampap2L, nextprime(392 * totfactor), msize, 0.312f,
                decay3);
  allpassm_make(&rv->dampap2R, nextprime(329 * totfactor), msize, 0.312f,
                decay3);

  allpass2_make(&rv->cbassap1L, nextprime(1944 * totfactor),
                nextprime(612 * totfactor), 0.250f, 0.406f, decay1, decay2);
  allpass2_make(&rv->cbassap1R, nextprime(2032 * totfactor),
                nextprime(368 * totfactor), 0.250f, 0.406f, decay1, decay2);

  allpass3_make(&rv->cbassap2L, nextprime(1212 * totfactor),
                nextprime(121 * totfactor), nextprime(816 * totfactor),
                nextprime(1264 * totfactor), 0.250f, 0.250f, 0.406f, decay1,
                decay1, decay2);
  allpass3_make(&rv->cbassap2R, nextprime(1452 * totfactor),
                nextprime(5 * totfactor), nextprime(688 * totfactor),
                nextprime(1340 * totfactor), 0.250f, 0.250f, 0.406f, decay1,
                decay1, decay2);

  static const int outco[32] = {1,   40,  192, 276, 321, 110,  468, 1572,
                                121, 480, 103, 26,  780, 1200, 310, 780,
                                625, 468, 312, 24,  36,  790,  189, 8,
                                10,  359, 30,  10,  109, 1310, 800, 10};
  for (int i = 0; i < 32; i++)
...
void sf_reverb_process(sf_reverb_state_st *rv, int size, sf_sample_st *input,
                       sf_sample_st *output) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    for (int i2 = 0; i2 < rv->oversampleL.factor; i2++) {
      ...
      outL +=
          rv->loopdecay *
          (crossR + rv->bassb * biquad_step(&rv->basslpL,
                                            biquad_step(&rv->bassapL, crossR)));
      outR +=
          rv->loopdecay *
          (crossL + rv->bassb * biquad_step(&rv->basslpR,
                                            biquad_step(&rv->bassapR, crossL)));

      // dampening
      outL = allpassm_step(
          &rv->dampap2L,
          delay_step(&rv->dampdL,
                     allpassm_step(&rv->dampap1L, iir1_step(&rv->damplpL, outL),
                                   lfo, mnoise)),
          -lfo, -mnoise);
      outR = allpassm_step(
          &rv->dampap2R,
          delay_step(&rv->dampdR,
                     allpassm_step(&rv->dampap1R, iir1_step(&rv->damplpR, outR),
                                   -lfo, -mnoise)),
          lfo, mnoise);
      ...
      //
      float D1 = delay_get(&rv->cbassd1L, rv->outco[0]);
      float D2 = delay_get(&rv->cbassd2L, rv->outco[1]) -
      ...
      float D3 = delay_get(&rv->cdelayL, rv->outco[7]) +
      ...
      float D4 = delay_get(&rv->cdelayL, rv->outco[15]);

      float B1 = delay_get(&rv->cbassd1R, rv->outco[16]);
      float B2 = delay_get(&rv->cbassd2R, rv->outco[17]) -
      ...
      float B3 = delay_get(&rv->cdelayR, rv->outco[23]) +
      ...
      float B4 = delay_get(&rv->cdelayR, rv->outco[31]);
