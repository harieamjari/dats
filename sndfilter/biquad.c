...
// biquad filtering is based on a small sliding window, where the different
// filters are a result of simply changing the coefficients used while
// processing the samples
//
// the biquad filter processes a sound using 10 parameters:
//   b0, b1, b2, a1, a2      transformation coefficients
//   xn0, xn1, xn2           the unfiltered sample at position x[n], x[n-1], and
//   x[n-2] yn1, yn2                the filtered sample at position y[n-1] and
//   y[n-2]
void sf_biquad_process(sf_biquad_state_st *state, int size, sf_sample_st *input,
  ...
  for (int n = 0; n < size; n++) {
    ...
    float L = b0 * xn0.L + b1 * xn1.L + b2 * xn2.L - a1 * yn1.L - a2 * yn2.L;
    float R = b0 * xn0.R + b1 * xn1.R + b2 * xn2.R - a1 * yn1.R - a2 * yn2.R;
...
//
// the math is quite complicated to understand, but the *implementation* is
// quite simple
//
// I have no insight into the genius of the math -- you're on your own for that.
// You might find some help in some of the articles here:
...
void sf_lowpass(sf_biquad_state_st *state, int rate, float cutoff,
                float resonance) {
  state_reset(state);
  ...
  if (cutoff >= 1.0f)
    ...
    resonance =
        powf(10.0f, resonance * 0.05f); // convert resonance from dB to linear
...
void sf_highpass(sf_biquad_state_st *state, int rate, float cutoff,
                 float resonance) {
  state_reset(state);
  float nyquist = rate * 0.5f;
  cutoff /= nyquist;
...
void sf_peaking(sf_biquad_state_st *state, int rate, float freq, float Q,
                float gain) {
  state_reset(state);
  float nyquist = rate * 0.5f;
  freq /= nyquist;
  ...
  float A = powf(
      10.0f, gain * 0.025f); // square root of gain converted from dB to linear
...
void sf_lowshelf(sf_biquad_state_st *state, int rate, float freq, float Q,
                 float gain) {
  state_reset(state);
  float nyquist = rate * 0.5f;
  freq /= nyquist;
...
// WebAudio hardcodes Q=1
void sf_highshelf(sf_biquad_state_st *state, int rate, float freq, float Q,
                  float gain) {
  state_reset(state);
  float nyquist = rate * 0.5f;
  freq /= nyquist;
