...
static int printabout() {
  printf("sndfilter - simple demonstrations of common sound filters\n"
         "(c) Copyright 2016, Sean Connelly (@velipso), https://sean.cm\n"
         "MIT License\n"
         "Project Home: https://github.com/velipso/sndfilter\n");
  return 0;
...
static int printhelp() {
  ...
  printf(
      ...
      "\n"
      "  Filters:\n"
      "    lowpass     Passes low frequencies through and dampens high "
      "frequencies\n"
      "    highpass    Passes high frequencies through and dampens low "
      "frequencies\n"
      "    bandpass    Passes a band of frequencies through and dampens "
      "outside them\n"
      ...
      "    compressor  Dyanmic range compression, usually to make sounds "
      "louder\n"
      ...
      "\n"
      "    allpass <freq> <Q>\n"
      "      freq       Frequency where the phase shift crosses 90 degrees "
      "(Hz)\n"
      "      Q          Inverse of the fractional bandwidth\n"
      "\n"
      "    lowshelf <freq> <Q> <gain>\n"
      "      freq       Frequency where the low shelf stops producing gain "
      "(Hz)\n"
      "      Q          Inverse of the fractional bandwidth\n"
      "      gain       Amount of gain of the shelf (dB)\n"
      "\n"
      "    highshelf <freq> <Q> <gain>\n"
      "      freq       Frequency where the high shelf stops producing gain "
      "(Hz)\n"
      "      Q          Inverse of the fractional bandwidth\n"
      "      gain       Amount of gain of the shelf (dB)\n"
      "\n"
      "    compressor <pregain> <threshold> <knee> <ratio> <attack> <release>\n"
      "      pregain    Decibel amount to perform gain before compression (0 "
      "to 40)\n"
      "      threshold  Decibel level that triggers the compression (-100 to "
      "0)\n"
      ...
      "                   mediumroom1, mediumroom2, largeroom1, largeroom2, "
      "mediumer1,\n"
      "                   mediumer2, platehigh, platelow, longreverb1, "
      "longreverb2\n");
  return 0;
}
...
static inline int biquad(sf_snd input_snd, sf_biquad_state_st *state,
                         const char *output) {
  ...
  sf_biquad_process(state, input_snd->size, input_snd->samples,
                    output_snd->samples);
...
static inline int compressor(sf_snd input_snd, sf_compressor_state_st *state,
                             const char *output) {
  ...
  sf_compressor_process(state, input_snd->size, input_snd->samples,
                        output_snd->samples);

  // note that the compressor does not output one sample per input sample,
  // because the compressor works in subchunks of 32 samples (this is defined
  // via SF_COMPRESSOR_SPU in compressor.c)
  //
...
static inline int reverb(sf_snd input_snd, float tail, const char *preset,
                         const char *output) {
  sf_reverb_preset p;
  if (strcmp(preset, "default") == 0)
    p = SF_REVERB_PRESET_DEFAULT;
  else if (strcmp(preset, "smallhall1") == 0)
    p = SF_REVERB_PRESET_SMALLHALL1;
  else if (strcmp(preset, "smallhall2") == 0)
    p = SF_REVERB_PRESET_SMALLHALL2;
  else if (strcmp(preset, "mediumhall1") == 0)
    p = SF_REVERB_PRESET_MEDIUMHALL1;
  else if (strcmp(preset, "mediumhall2") == 0)
    p = SF_REVERB_PRESET_MEDIUMHALL2;
  else if (strcmp(preset, "largehall1") == 0)
    p = SF_REVERB_PRESET_LARGEHALL1;
  else if (strcmp(preset, "largehall2") == 0)
    p = SF_REVERB_PRESET_LARGEHALL2;
  else if (strcmp(preset, "smallroom1") == 0)
    p = SF_REVERB_PRESET_SMALLROOM1;
  else if (strcmp(preset, "smallroom2") == 0)
    p = SF_REVERB_PRESET_SMALLROOM2;
  else if (strcmp(preset, "mediumroom1") == 0)
    p = SF_REVERB_PRESET_MEDIUMROOM1;
  else if (strcmp(preset, "mediumroom2") == 0)
    p = SF_REVERB_PRESET_MEDIUMROOM2;
  else if (strcmp(preset, "largeroom1") == 0)
    p = SF_REVERB_PRESET_LARGEROOM1;
  else if (strcmp(preset, "largeroom2") == 0)
    p = SF_REVERB_PRESET_LARGEROOM2;
  else if (strcmp(preset, "mediumer1") == 0)
    p = SF_REVERB_PRESET_MEDIUMER1;
  else if (strcmp(preset, "mediumer2") == 0)
    p = SF_REVERB_PRESET_MEDIUMER2;
  else if (strcmp(preset, "platehigh") == 0)
    p = SF_REVERB_PRESET_PLATEHIGH;
  else if (strcmp(preset, "platelow") == 0)
    p = SF_REVERB_PRESET_PLATELOW;
  else if (strcmp(preset, "longreverb1") == 0)
    p = SF_REVERB_PRESET_LONGREVERB1;
  else if (strcmp(preset, "longreverb2") == 0)
  ...
  sf_snd output_snd =
      sf_snd_new(input_snd->size + tailsmp, input_snd->rate, true);
  if (output_snd == NULL) {
    sf_snd_free(input_snd);
    fprintf(stderr, "Error: Failed to apply filter\n");
    return 1;
  ...
  sf_reverb_process(&rv, input_snd->size, input_snd->samples,
                    output_snd->samples);
...
int main(int argc, char **argv) {
  ...
  if (strcmp(filter, "lowpass") == 0) {
    ...
    sf_simplecomp(&cm_state, input_snd->rate, params[0], params[1], params[2],
