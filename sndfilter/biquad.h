...
//
// this API works by first initializing an sf_biquad_state_st structure, and
// then using it to process a sample in chunks
//
// for example, for a lowpass filter over a stream with 128 samples per chunk,
// you would do:
//
...
//
// notice that sf_biquad_process will change the xn1,xn2,yn1,yn2 values inside
// of the state structure, since these values must be carried over across chunk
// boundaries
//
// also notice that the choice to divide the sound into chunks of 128 samples is
// completely arbitrary from the filter's perspective
...
// these functions will initialize an sf_biquad_state_st structure based on the
// desired filter
