...
sf_snd sf_wavload(const char *file) {
  ...
  while (!feof(fp)) {
    ...
    if (chunkid == 0x20746D66) { // 'fmt '
      ...
      // confirm we haven't already processed the fmt chunk, and that it's a
      // good size
      ...
      // calculate the number of samples based on the chunk size and allocate
      // the space
      ...
      for (int i = 0; i < scount; i++) {
        ...
        // notice that int16 samples range from -32768 to 32767, therefore we
        // have a different divisor depending on whether the value is negative
        // or not
        if (L < 0)
...
bool sf_wavsave(sf_snd snd, const char *file) {
  ...
  for (int i = 0; i < snd->size; i++) {
    ...
    int16_t Lv, Rv;
    // once again, int16 samples range from -32768 to 32767, so we need to scale
    // the floating point sample by a different factor depending on whether it's
    // negative
    if (L < 0)
