...
// core algorithm extracted from Chromium source, DynamicsCompressorKernel.cpp,
// here:
//   https://git.io/v1uSK
//
// changed a few things though in an attempt to simplify the curves and
// algorithm, and also included a pregain so that samples can be scaled up then
// compressed
...
void sf_simplecomp(sf_compressor_state_st *state, int rate, float pregain,
                   float threshold, float knee, float ratio, float attack,
                   float release) {
  // sane defaults
...
// for more information on the knee curve, check out the compressor-curve.html
// demo + source code included in this repo
...
static inline float kneeslope(float x, float k, float linearthreshold) {
  return k * x /
         ((k * linearthreshold + 1.0f) * expf(k * (x - linearthreshold)) - 1);
}

static inline float compcurve(float x, float k, float slope,
                              float linearthreshold, float linearthresholdknee,
                              float threshold, float knee, float kneedboffset) {
...
// it does a bunch of pre-calculation so that the inner loop of signal
// processing is fast
void sf_advancecomp(sf_compressor_state_st *state, int rate, float pregain,
                    float threshold, float knee, float ratio, float attack,
                    float release, float predelay, float releasezone1,
                    float releasezone2, float releasezone3, float releasezone4,
  ...
  // metering values (not used in core algorithm, but used to output a meter if
  // desired)
  float metergain =
      1.0f; // gets overwritten immediately because gain will always be negative
  ...
  if (knee > 0.0f) { // if a knee exists, search for a good k value
    ...
    float mink = 0.1f;
    float maxk = 10000.0f;
    // search by comparing the knee slope at the current k guess, to the ideal
    // slope
    for (int i = 0; i < 15; i++) {
  ...
  float fulllevel =
      compcurve(1.0f, k, slope, linearthreshold, linearthresholdknee, threshold,
                knee, kneedboffset);
  ...
  // save everything
  state->metergain =
      1.0f; // large value overwritten immediately since it's always < 0
...
// for more information on the adaptive release curve, check out
// adaptive-release-curve.html demo + source code included in this repo
static inline float adaptivereleasecurve(float x, float a, float b, float c,
                                         float d) {
  // a*x^3 + b*x^2 + c*x + d
  float x2 = x * x;
...
static inline float fixf(float v, float def) {
  // fix NaN and infinity values that sneak in... not sure why this is needed,
  // but it is
  if (isnan(v) || isinf(v))
    return def;
  return v;
...
void sf_compressor_process(sf_compressor_state_st *state, int size,
                           sf_sample_st *input, sf_sample_st *output) {
  ...
  for (int ch = 0; ch < chunks; ch++) {
    ...
    for (int chi = 0; chi < samplesperchunk; chi++, samplepos++,
      ...
      if (inputmax < 0.0001f)
        ...
        float inputcomp =
            compcurve(inputmax, k, slope, linearthreshold, linearthresholdknee,
      ...
      // calculate metering (not used in core algo, but used to output a meter
      // if desired)
      ...
      // apply the gain
      output[samplepos] = (sf_sample_st){.L = delaybuf[delayreadpos].L * gain,
